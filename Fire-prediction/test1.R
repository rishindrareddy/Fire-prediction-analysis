...
# getting the map
mapgilbert <- get_map(location = c(lon = mean(df$lon), lat = mean(df$lat)), zoom = 1,
                      maptype = "satellite", scale = 2)

# plotting the map with some points on it
ggmap(mapgilbert) +
  geom_point(data = df, aes(x = myData1$Long, y = myData1$Lat, fill = myData1$fire, alpha = 0.8), size = 5, shape = 21) +
